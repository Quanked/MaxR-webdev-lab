...
const mapNameFamily = (characters) => {
  ...
  const person = characters.map(
    (person) => `${person.name} of House ${person.house}`
  );
...
const filterFamily = (characters, house) => {
  ...
  const person = characters.filter((characters) => characters.house === house);
...
const reduceHouses = (characters) => {
  ...
  const number = characters.reduce((count, person) => {
    if (count[person.house]) {
      count[person.house]++;
    } else {
      count[person.house] = 1;
    }
    return count;
  }, {});
